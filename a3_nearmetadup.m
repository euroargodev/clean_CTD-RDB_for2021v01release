    ...
    for j=1:numel(boxlist)
        ...
        for k=1:n
            ...
            if sum(ismember(ind(k,:),excl))==0
                 % checks if the profile is deep content duplicate (at least 90% of match below 900 m) 
                [perc_t(k,1),perc_s(k,1),conf(k,1)]=prof_compcontdeep(filein,ind(k,:),900,95);
...
save a3_results.mat boxes output* regions EXCL PER* IND;