...
%
% Output:
% W index of the worst profile (depending on the combination of WQCL and WSCR
% WQCL is a 1 or a 2 if there if the first or second profile is worst than
% the other taking into account its qclevel. Or 0 if there is no way to
% decide if one of the profiles is better taking into accoun the qclevel
% only
% WSCR like WQCL but taking into account the profile's sources
...
if isempty(filein)||isempty(ind)
    ...
    %if there is no info in the prof assign 0 to all the values.
    % prova Anto se non ho data.qclevel
    if isfield(data, 'qclevel')==0|| isfield(data, 'source')==0
        w=0;wqcl=0;wsrc=0;d=0;
    else % otherwise evaluates the qclevel and the source
